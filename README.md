...
- [Карта модулей и потоков данных](docs/architecture.md)
- [Разработка и единая команда проверки](docs/development.md)
- [Шаблон спецификации задачи](docs/spec-template.md)
...
Названия групп веб получает через Telegram API и кэширует в `web_chat_titles`; если название недоступно, показывает `Чат <chat_id>`. Для личного чата используется `ЛС`.
