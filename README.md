...
Задача `response` генерирует живые ответы бота в чат: по прямому обращению, упоминанию `@udb_flood_bot`, слову "бот" или случайно с шансом из `/settings` (по умолчанию 3%, максимум 10%). Ответ строится на последних сообщениях, последних саммари и профиле автора.
