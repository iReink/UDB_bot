...
Веб-питомец игрока в конкретном чате. В v1 используется только веб-сценой: яйцо с 5 стадиями и вылупившийся кролик.
...
| `state` | TEXT | Состояние питомца: `egg` или `rabbit`. |
...
| `hatched_at` | TEXT | Дата-время вылупления кролика, `NULL` пока питомец в яйце. |
