...
POLL_SECONDS = float(os.getenv("AI_WORKER_POLL_SECONDS", "5"))
OLLAMA_TIMEOUT_SECONDS = int(os.getenv("AI_WORKER_OLLAMA_TIMEOUT_SECONDS", "300"))
BACKEND_TIMEOUT_SECONDS = int(os.getenv("AI_WORKER_BACKEND_TIMEOUT_SECONDS", "30"))
...
def get_next_task() -> dict[str, Any] | None:
    response = request_json(
        f"{BACKEND_URL}/api/ai/tasks/next",
...
def post_result(task_id: int, *, output: str = "", error: str = "") -> dict[str, Any]:
    return request_json(
        f"{BACKEND_URL}/api/ai/tasks/{task_id}/result",
        method="POST",
...
def process_task(task: dict[str, Any]) -> None:
    ...
    log(f"task #{task_id}: {task.get('task_type')} model={model}")
    try:
        output = call_ollama(model, prompt)
    except Exception as exc:
        log(f"task #{task_id}: Ollama error: {exc}")
        post_result(task_id, error=str(exc))
        return

    if not output:
        log(f"task #{task_id}: empty Ollama response")
    ...
    try:
        ...
        log(f"task #{task_id}: backend result POST failed: {exc}")
        return
    log(f"task #{task_id}: backend status={response.get('status')}")
...
def main() -> int:
    ...
    log(f"worker started: backend={BACKEND_URL}, ollama={OLLAMA_URL}, poll={POLL_SECONDS}s")
    while True:
        try:
            task = get_next_task()
            if task:
