...
def register_daily_handlers(dp: Dispatcher):
    # Inline-кнопки для выбора машины при создании
    ...
    cancel_kb.add(InlineKeyboardButton(text="❌ Отмена", callback_data="daily_cancel"))

    # ==========================
    # FSM: создание нового дейлика
    # ==========================
    @dp.callback_query(lambda c: c.data == "daily_new_daily")
    async def start_creation(query: types.CallbackQuery, state: FSMContext):
        ...
        await query.message.answer("Введите название дейлика:", reply_markup=cancel_kb.as_markup())
        await query.answer("Создание дейлика начато ✅")

    @dp.callback_query(lambda c: c.data == "daily_cancel")
    async def cancel_creation(query: types.CallbackQuery, state: FSMContext):
        await state.clear()
        chat_id = query.message.chat.id
        active_creators.pop(chat_id, None)
        await query.message.answer("Создание дейлика отменено ❌", reply_markup=ReplyKeyboardRemove())
        await query.answer()

    @dp.message(lambda m: True)
    async def handle_fsm_messages(message: types.Message, state: FSMContext):
        ...
        if text == "❌ Отмена":
            await state.clear()
            active_creators.pop(chat_id, None)
            await message.answer("Создание дейлика отменено ❌", reply_markup=ReplyKeyboardRemove())
            return

        data = await state.get_data()
        if current_state.endswith("name"):
            await state.update_data(name=text)
            await state.set_state(CreateDaily.description)
            await message.answer("Введите описание дейлика:", reply_markup=cancel_kb.as_markup())
        elif current_state.endswith("description"):
            await state.update_data(description=text)
            await state.set_state(CreateDaily.datetime)
            await message.answer("Введите дату и время (ДД.ММ ЧЧ:ММ):", reply_markup=cancel_kb.as_markup())
        elif current_state.endswith("datetime"):
            try:
                ...
                await message.answer("Неверный формат. Пример: 17.07 19:00")
                return
            await state.set_state(CreateDaily.link)
            await message.answer("Введите ссылку или '-' если нет:", reply_markup=cancel_kb.as_markup())
        elif current_state.endswith("link"):
            link = text.strip()
            if link in ("-", "–", "—"):
                link = None
            await state.update_data(link=link)
            await state.set_state(CreateDaily.cars)
            await message.answer("Нужно ли ехать на машине?", reply_markup=cars_kb.as_markup())

    @dp.callback_query(lambda c: c.data in ("daily_cars_yes", "daily_cars_no"))
    async def handle_cars_choice(query: types.CallbackQuery, state: FSMContext):
        ...
        await query.message.answer("📆 Дейлик создан!", reply_markup=ReplyKeyboardRemove())
    ...
    # ==========================
    # Callback-обработчик старых кнопок
    # ==========================
    ...
    async def daily_callback(query: types.CallbackQuery, state: FSMContext):
        ...
        # ==========================
        # Работа с кнопками join/leave/driver/nodriver
        # ==========================
