...
def ensure_user(chat_id, user_id, user_name):
    ...
    Приводит daily к длине 7 и добавляет поле 'stickers' в day и total.
    """
    chat_id = str(chat_id)
    user_id = str(user_id)
    ...
    if user_id not in stats[chat_id]:
        ...
        stats[chat_id][user_id] = {
            "name": user_name,
            "daily": [{"messages": 0, "words": 0, "chars": 0, "stickers": 0} for _ in range(7)],
            "total": {"messages": 0, "words": 0, "chars": 0, "stickers": 0}
        }
    ...
    if not isinstance(daily, list):
        ...
        if len(daily) < 7:
            for _ in range(7 - len(daily)):
                daily.append({"messages": 0, "words": 0, "chars": 0, "stickers": 0})
...
def migrate_stats_add_stickers():
    """
    Пройти по существующему stats и добавить поле 'stickers' в daily и total,
    если его нет — для обратной совместимости.
    """
    changed = False
    ...
    if changed:
        save_stats(stats)
        logging.info("Миграция: добавлены поля 'stickers' в stats (если нужно).")


stats = load_stats()
migrate_stats_add_stickers()
...
def shift_daily_stats():
    for chat_id in stats:
        for user_id in stats[chat_id]:
            ...
            daily.pop(-1)
            daily.insert(0, {"messages": 0, "words": 0, "chars": 0, "stickers": 0})
    save_stats(stats)
